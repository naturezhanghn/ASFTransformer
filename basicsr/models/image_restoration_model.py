...
class ImageMlossModel(BaseModel):
    ...
    def init_training_settings(self):
        ...
        if train_opt.get('phase_opt'):
            pixel_type = train_opt['phase_opt'].pop('type')
            cri_flow_cls = getattr(loss_module, pixel_type)
            self.cri_phase = cri_flow_cls(**train_opt['phase_opt']).to(
                self.device)
            print( "Phase loss loaded!")
    ...
    def optimize_parameters(self, current_iter):
        ...
        for pred in preds:
            ...
            if self.opt['train'].get('phase_opt'):
                l_pix += self.cri_phase(pred, self.gt)
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger,
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            if save_img:
                ...
                imwrite(sr_img, save_img_path)
                imwrite(gt_img, save_gt_img_path)
                imwrite(tensor2img(val_data["lq"][:,0:3,:,:]), save_lq_img_path  )
...
class ImageMlossFlowModel(BaseModel):
    ...
    def feed_train_data(self, data):
        ...
        if "flow_gt" in data:
            self.flowgt = data['flow_gt'].to(self.device)#.permute(0,2,3,1).contiguous()
        
        if "flow" in data:
            self.flow = data['flow'].to(self.device)#.permute(0,2,3,1).contiguous()
    ...
    def optimize_parameters(self, current_iter):
        self.optimizer_g.zero_grad()
        # print("12230033",self.lq.shape,self.flow.shape)
        ...
        for preflow in preflows:
            # print("loss",preflow.shape, self.flowgt.shape)
            l_pix += self.cri_pix(preflow, self.flowgt.permute(0,2,3,1).contiguous())
