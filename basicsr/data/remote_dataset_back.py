...
class REMOTEDATASet(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        gt = data_dict["img_gt"]
        flow = data_dict["jit_information"]

        img = np.expand_dims(img, axis=2)
        gt = np.expand_dims(gt, axis=2)
        
        # normalization
        img = imfrombytes(img , flag='grayscale',float32=True)
        gt = imfrombytes(gt , flag='grayscale',float32=True) 
        ...
        # print(img.shape, gt.shape, flow.shape)
        return {'lq': img, 'gt': gt, 'flow': flow,"lq_path": self.path[index % len(self.path)]}
